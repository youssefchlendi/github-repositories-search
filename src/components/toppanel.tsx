...
const TopPanel = () => {
	...
	return (
		<div className="container">
			<h1 className="leftPart">{userName}</h1>
			<div className="navMenu">
				{Items.map((val, index) => (
					<Link to={val.link} key={index} className={`navItem navItemUnderline ${item.id == val.id ? "active" : ""} `} onClick={() => setItem(val)}>
						<svg aria-hidden="true" height="16" viewBox="0 0 16 16" version="1.1" width="16" data-view-component="true" className="navItemIcon" dangerouslySetInnerHTML={{ __html: val.svg }}>
						</svg>
						{val.text}
					</Link>
				))}
